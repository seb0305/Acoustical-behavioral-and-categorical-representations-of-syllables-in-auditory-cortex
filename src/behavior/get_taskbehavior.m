...
%           - then mean-centered
%           - then scaled to [-1, 1] by dividing by max absolute value.
%
% EXAMPLE
...
% See also: build_behavioral_model (if you wrap this into a larger pipeline).
    ...
    % Vowel is 1st dimension in behav, speaker is 2nd dimension in behav.
    ...
    % Mean-center behavioral labels.
    BMAT = BMAT - mean(BMAT);

    % Scale to [-1, 1] by dividing by max absolute value (if non-zero).
    maxAbs = max(abs(BMAT));
    if maxAbs > 0
        BMAT = BMAT ./ maxAbs;
    end
